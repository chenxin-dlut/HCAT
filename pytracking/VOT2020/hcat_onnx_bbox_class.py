...
def run_vot_exp(name, window, penalty_k, backbone_path,model_path, save_root, VIS=False):
    torch.set_num_threads(1)
    # torch.cuda.set_device(CUDA_ID)  # set GPU id
    if VIS and (not os.path.exists(save_root)):
        os.mkdir(save_root)

    tracker = HCAT(name=name, backbone_path=backbone_path,model_path=model_path, feature_size=16, window_penalty=window, penalty_k=penalty_k, exemplar_size=128, instance_size=256)
